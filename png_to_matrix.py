...
def pad_matrix(matrix: np.ndarray, padding_percent: float):
    ...
    """
    assert padding_percent >= 0 and padding_percent <= 1, "Padding percent must be between 0 and 1"
    padding0 = int(matrix.shape[0] * padding_percent)
    padding1 = int(matrix.shape[1] * padding_percent)
    return np.pad(matrix, ((padding0, padding0), (padding1, padding1)), mode='constant', constant_values=255)
